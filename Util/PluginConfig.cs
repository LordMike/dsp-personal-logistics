...
namespace PersonalLogistics.Util
{
    public enum StationSourceMode
    {
        ...
        [Description(
            "Only interact with logistics stations on the same planet as the player. Follow the same rules as a nearby PLS set to Demand")]
        Planetary
    }
    ...
    public enum PlanetarySourceMode
    {
        ...
        [Description(
            "When you go to a new planet, buffered items can still be used to refill inventory, but <b>only</b> if the item is also available on the new planet")]
    ...
    public class PluginConfig
    {
        ...
        public static void InitConfig(ConfigFile confFile)
        {
            ...
            warpEnableMinAu = confFile.Bind("Logistics", "Warp Enable Min AU", 1,
                new ConfigDescription(
                    "Set a minimum in AU (1 AU = 40 KM, 1 LY = 60 AU) where warpers will be required before shipping will be attempted\r\n" +
            ...
            maxWaitTimeInSeconds = confFile.Bind("Inventory", "Max Wait Time In Seconds", 600,
                new ConfigDescription(
                    "Max time to wait for items to be delivered. If calculated arrival time is more than this value, item request will be canceled",
                         ...
                         .FindAll(i => i.ID > 0 && i.FuelType > 0))
            {
                enabledFuelItems.Add(fuelItemProto.ID,
                    confFile.Bind("Internal", $"FuelType_{fuelItemProto.Name.Translate()}_Enabled", true,
                        $"Allow {fuelItemProto.name} to be used in mecha fuel chamber"));
            }
            ...
            testOverrideLanguage = confFile.Bind("Internal", "TEST override language", "",
                new ConfigDescription("Force an alt language to be used (for some text)",
                    new AcceptableValueList<string>(
                        Localization.Languages.Select(s => s.abbr).ToArray()
                    )));
        ...
        {
            return inventoryManagementPaused.Value || !GameUtil.IsPlayerGameRunning();
        }
